...
class SubdomainMiddleware(object):
	...
	if len(request.get_host().split('.')) > 2 and request.get_host().split('.')[0] != "app" and request.user.is_authenticated():
		...
		uzer = str(request.user).lower()
	        return HttpResponse("You are logged in as " + uzer + ", and your subdomain is " + subdomain + ". Please go to " + uzer + ".trackpattern.com")
	...
	elif len(request.get_host().split('.')) > 2 and request.get_host().split('.')[0] != "app" and request.user.is_anonymous():
