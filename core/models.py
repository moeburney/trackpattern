...
class Group():
    ...
    def get_group(self, id):
        id = int(id)
        if id < 2:
            customers = Customer.objects.filter(user=self.user).annotate(bought=Count('purchase')).filter(bought=id)
        else:
            customers = Customer.objects.filter(user=self.user).annotate(bought=Count('purchase')).filter(bought__gte=id)
...
class Category(models.Model):
    ...
    def total_revenue_generated(self):
        ...
        """
        turnover = Purchase.objects.filter(customer__category=self).aggregate(turnover=Sum('price'))
        return turnover['turnover'] or 0.0
        

    def total_purchases(self):
        return Purchase.objects.filter(customer__category=self).count()

    def most_common_path(self):
        most_bought_products = Product.objects.filter(purchase__customer__category=self).annotate(bought=Count('purchase__product')).order_by('-bought')
        return ' > '.join(product.name for product in most_bought_products)
    
    def get_rank(self):
        groups_by_revenue = Category.objects.filter(user=self.user).annotate(revenue=Sum('customer__purchase__price')).order_by('-revenue')
        counter = 0
    ...
    def last_activity(self):
        try:
            purchase = Purchase.objects.filter(customer__category=self).order_by('-transaction_date')[0]
...
class Product(models.Model):
    ...
    def most_bought_category(self):
        ...
        """
        categories = Category.objects.filter(customer__purchase__product=self).annotate(bought=Count('customer__purchase__product')).order_by('-bought')
        if categories:
...
class Customer(models.Model):
    ...
    def group(self):
        ...
        """
        purchase_count = self.purchase_set.count()
